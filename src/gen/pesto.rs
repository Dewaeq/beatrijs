...
const fn gen_mg_pesto() -> [[Score; NUM_SQUARES]; NUM_PIECES * 2] {
    ...
    while piece < NUM_PIECES {
        let mut sq = 0;
        while sq < 64 {
            table[piece][sq] = MG_VALUE[piece] + MG_PIECE_TABLE[piece][sq];
            table[piece + 6][sq] = MG_VALUE[piece] + MG_PIECE_TABLE[piece][mirror(sq as Square) as usize];
...
const fn gen_eg_pesto() -> [[Score; NUM_SQUARES]; NUM_PIECES * 2] {
    ...
    while piece < NUM_PIECES {
        let mut sq = 0;
        while sq < 64 {
            table[piece][sq] = EG_VALUE[piece] + EG_PIECE_TABLE[piece][sq];
            table[piece + 6][sq] = EG_VALUE[piece] + EG_PIECE_TABLE[piece][mirror(sq as Square) as usize];
